...
public class TextDiff extends AbstractDiffClient<TextDiff.TextDiffOptions> {
    ...
    public static class TextDiffOptions extends AbstractDiffClient.DiffOptions {
        ...
        protected String diffAlgorithm;
        ...
        public Option[] values() {
            return Option.Context.addValue(super.values(),
                    ...
                    new Option("-d", String.format("diffAlgorithm: %s", Arrays.toString(OutputFormat.values())), 1) {
                        @Override
                        protected void process(String name, String[] args) {
                            diffAlgorithm = args[0];
                        }
                    },
    ...
    public void run() throws Exception {
        Diff diff = Diff.compute(opts.srcPath, opts.dstPath, opts.treeGeneratorId, opts.matcherId, opts.properties, opts.diffAlgorithm);
        TreeIoUtils.AbstractSerializer serializer = opts.format.getSerializer(
                diff.src, diff.editScript, diff.mappings, diff.timings);
        if (opts.output == null)
    ...
    enum OutputFormat {
        TEXT {
            @Override
            TreeIoUtils.AbstractSerializer getSerializer(TreeContext sctx, EditScript actions, MappingStore mappings, List<Long> times)
                    throws IOException {
                return ActionsIoUtils.toText(sctx, actions, mappings, times);
            }
        },
        XML {
            @Override
            TreeIoUtils.AbstractSerializer getSerializer(TreeContext sctx, EditScript actions, MappingStore mappings, List<Long> times)
                    throws IOException {
                return ActionsIoUtils.toXml(sctx, actions, mappings, times);
            }
        },
        JSON {
            @Override
            TreeIoUtils.AbstractSerializer getSerializer(TreeContext sctx, EditScript actions, MappingStore mappings, List<Long> times)
                    throws IOException {
                return ActionsIoUtils.toJson(sctx, actions, mappings, times);
            }
        },
        COMPRESSED {
            @Override
            TreeIoUtils.AbstractSerializer getSerializer(TreeContext sctx, EditScript actions, MappingStore mappings, List<Long> times)
                    throws IOException {
                return ActionsIoUtils.toCompressed(sctx, actions, mappings, times);
            }
        };

        abstract TreeIoUtils.AbstractSerializer getSerializer(TreeContext sctx, EditScript actions,
