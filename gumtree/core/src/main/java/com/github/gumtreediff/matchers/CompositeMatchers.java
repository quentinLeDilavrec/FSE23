...
public class CompositeMatchers {
    public static class CompositeMatcher implements ConfigurableMatcher, TimedMatcher {
        ...
        public Timed timedMatch(Tree src, Tree dst, MappingStore mappings) {
            ...
            for (Matcher matcher : matchers) {
                if (matcher instanceof TimedMatcher) {
                    throw new UnsupportedOperationException("not implemented");
                } else {
                    long t = System.nanoTime();
                    mappings = matcher.match(src, dst, mappings);
                    l.add(System.nanoTime() - t);
                }
            }
            return new Timed(mappings, l);
        }
    ...
    @Register(id = "gumtree-subtree", priority = Registry.Priority.MAXIMUM)
    public static class ClassicGumtreeSubtree extends CompositeMatcher {
        public ClassicGumtreeSubtree() {
    ...
    @Register(id = "gumtree-simple-id2", priority = Registry.Priority.HIGH)
    public static class SimpleIdGumtree2 extends CompositeMatcher {
        public SimpleIdGumtree2() {
            super(new GreedySubtreeMatcher(), new IdMatcher(), new SimpleBottomUpMatcher());
        }
    }

    @Register(id = "gumtree-simple-id3", priority = Registry.Priority.HIGH)
    public static class SimpleIdGumtree3 extends CompositeMatcher {
        public SimpleIdGumtree3() {
            super(new GreedySubtreeMatcher(), new SimpleBottomUpMatcher(), new IdMatcher());
        }
    }

    @Register(id = "gumtree-simple-id4", priority = Registry.Priority.HIGH)
    public static class SimpleIdGumtree4 extends CompositeMatcher {
        public SimpleIdGumtree4() {
            super(new GreedySubtreeMatcher(), new SimpleBottomUpMatcher(), new IdMatcher(),
                    new SimpleBottomUpMatcher());
        }
    }
