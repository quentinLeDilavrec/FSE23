...
public class SimilarityMetrics {
    ...
    public static int numberOfMappedDescendants(Tree src, Tree dst, MappingStore mappings) {

        Set<Tree> dstDescendants = new HashSet<>(dst.getDescendants());
        // if (dstDescendants.size() == 616 && src.getDescendants().size() == 609) {
        //     // // System.out.println(dst.getDescendants());
        //     // System.out.println(src.getDescendants());
        //     System.out.println("mappings.size()="+mappings.size());
        //     System.out.println(dstDescendants.size());
        //     System.out.println(src.getDescendants().size());
        // }
        ...
        for (var srcDescendant : src.getDescendants()) {
            if (mappings.isSrcMapped(srcDescendant)) {
                ...
                if (dstDescendants.contains(dstForSrcDescendant)) {
                    // if (dstDescendants.size() == 616 && src.getDescendants().size() == 609) {
                    //     System.out.println(i+" "+srcDescendant.getType()+" -> "+dstForSrcDescendant);
                    // }
                    mappedDescendants++;
                }
            }
