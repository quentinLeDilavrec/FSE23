...
public abstract class AbstractBottomUpMatcher implements Matcher {
    ...
    protected void lastChanceMatch(MappingStore mappings, Tree src, Tree dst) {
        if (src.getMetrics().size < sizeThreshold || dst.getMetrics().size < sizeThreshold) {
            ...
            // System.out.println("src_s = " + src.getMetrics().size + " dst_s = " + dst.getMetrics().size +" mappings="+ mappings.size() );
            MappingStore zsMappings = m.match(src, dst, new MappingStore(src, dst));
            // System.out.println("zs_mapping = " + zsMappings.size());
            if (src.getMetrics().size == 520 && dst.getMetrics().size == 527) {
                // int i = 0;
                // for (Tree s : src.postOrder()) {
                //     // System.out.println(i + " " + s + " -> "+ zsMappings.getDstForSrc(s));
                //     i++;
                // }
                // System.out.println(Arrays.toString(((ZsMatcher)m).treeDist[500]));
                // System.out.println(Arrays.toString(((ZsMatcher)m).treeDist[501]));
                // System.out.println(Arrays.toString(((ZsMatcher)m).treeDist[502]));
            }
