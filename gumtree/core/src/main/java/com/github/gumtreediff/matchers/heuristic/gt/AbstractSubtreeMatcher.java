...
public abstract class AbstractSubtreeMatcher implements Matcher {
    ...
    public MappingStore match(Tree src, Tree dst, MappingStore mappings) {
        ...
        while (!(srcTrees.isEmpty() || dstTrees.isEmpty())) {
            // aaa++;
            // System.out.println("multiMappings:" + multiMappings.size());
            ...
            // System.out.println(aaa + " marks:" + currentPrioritySrcTrees.size() +","+currentPriorityDstTrees.size());
            // int i = 0, j = 0;
                ...
                for (var currentDst : currentPriorityDstTrees) {
                    ...
                    if (currentSrc.getMetrics().hash == currentDst.getMetrics().hash)
                        if (currentSrc.isIsomorphicTo(currentDst)) {
                            // System.out.println(currentSrc.toTreeString());
                            // System.out.println(currentDst.toTreeString());
                            // System.out.println("isomorphic:" + i +","+j);
                            // System.out.println("children:" + currentSrc.getChildren().size() +","+currentDst.getChildren().size());
                            // System.out.println("pos:" + currentSrc.getPos() +","+currentSrc.getLength() +";"+currentDst.getPos() +","+currentDst.getLength() );
    ...
    protected void retainBestMapping(List<Mapping> mappingList, Set<Tree> srcIgnored, Set<Tree> dstIgnored) {
        while (mappingList.size() > 0) {
            ...
            // System.out.println("mapping:"+mapping.first.getPos()+" "+mapping.second.getPos());
